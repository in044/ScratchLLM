...
## 回答の契約
* 回答には、短い日本語の説明と、\`\`\`scratch\` で始まるScratchコードブロックをちょうど1個だけ含めてください。
* Scratchコードブロックには変更後のStageと全スプライトを含め、入力と同じ名前・同じ順番で各ターゲットをちょうど1回だけ出力してください。
* 見出しはStageなら \`# Stage\`、スプライトなら \`# 正確なスプライト名\` としてください。
* コードがないターゲットには、見出し直後に \`# ブロックなし\` とだけ書いてください。コードがあるターゲットではこの行を使ってはいけません。
* 変更しないスクリプトも一字一句の構文を保って出力してください。ユーザーが削除を依頼していないコードを省略しないでください。
* コードブロック内は1行に1ブロックとし、インデント、箇条書き、説明文、JSON、XML、opcode、省略記号を書かないでください。
...
1. Scratchコードブロックがちょうど1個で、全ターゲットの見出しが入力と同名・同順・各1回になっている。
2. 各ターゲットが、完全なコードまたは \`# ブロックなし\` のどちらか一方になっている。
3. 各コード行がリファレンスに存在する完全な1ブロックで、説明・省略・プレースホルダーがない。
4. すべての入力括弧が閉じ、空入力がなく、各メニューに \`v\` がある。
5. すべての条件入力が完全なBooleanブロックである。
6. C型制御ブロックと \`end\` の数・入れ子が一致し、各本体が空でない。
7. 変更しないコードが保持され、依頼されていない機能や素材が追加されていない。
8. 変数・x座標・y座標自身への加減算が「ずつ変える」になり、その入力に変更対象自身がない。固定数の減算は負数、動的な値の減算は \`((0) - (値))\` になっている。
9. Stageと各スプライトに、そのターゲットで使用できないブロックがない。
10. 最後にコード全体を上から1行ずつ再読し、曖昧な行をすべて修正した。
...
変更後の完成したプログラムを、全ターゲットの見出しを含む1個の \`\`\`scratch\` コードブロックで返してください。
